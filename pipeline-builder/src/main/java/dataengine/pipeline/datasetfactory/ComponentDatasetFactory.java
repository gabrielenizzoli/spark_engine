...
import dataengine.pipeline.datasetfactory.utils.Factories;
...
public class ComponentDatasetFactory implements DatasetFactory {
    ...
    private <T> Dataset<T> buildDataset(@Nonnull String name, @Nonnull List<String> childrenPath) throws DatasetFactoryException {
        ...
        if (childrenPath.contains(name))
            throw new DatasetFactoryException.CircularReference("component " + name + " already in path " + childrenPath + " (execution path has a circular reference)");
    ...
    private Component getComponent(String name) throws DatasetFactoryException {
        try {
            return componentCatalog.lookup(name).orElseThrow(() -> new DatasetFactoryException.ComponentNotFound(name));
    ...
    private <T> Dataset<T> getDataset(String name, Component component, List<String> childrenPath) throws DatasetFactoryException {
        ...
        if (ds == null)
            throw new DatasetFactoryException.ComponentNotManaged("component type [" + component.getClass().getName() + "] does not have any factory associated");
