...
public class Factories {
    ...
    public static <T> Dataset<T> getSourceComponentDataset(@Nonnull SourceComponent sourceComponent) throws DatasetBuilderException {
    ...
    public static <T> Dataset<T> getSingleInputComponent(TransformationComponentWithSingleInput singleInputComponent, Dataset<Object> inputDs) throws DatasetBuilderException {
    ...
    public static <T> Dataset<T> getMultiInputComponentDataset(TransformationComponentWithMultipleInputs multiInputComponent, List<Dataset<?>> parentDs) throws DatasetBuilderException {
        if (multiInputComponent instanceof Union) {
            return parentDs.stream()
                    .map(ds -> (Dataset<T>) ds)
                    .reduce(Dataset::union)
                    .orElseThrow(() -> new DatasetBuilderException("union can't be performed on an empty list of datasets"));
        }
    ...
    private static <T> Dataset<T> getTransformDataset(List<Dataset<?>> parentDs, Transform txComponent) throws DatasetBuilderException {
        ...
        try {
            ...
            throw new DatasetBuilderException("unable to instantiate transformation with class: [" + txComponent.getWith() + "]");
        }
    ...
    private static <T> Dataset<T> getSqlDataset(Sql sqlComponent, List<Dataset<?>> parentDs) throws DatasetBuilderException {
    ...
    private static <T> Dataset<T> getStreamDataset(StreamSource streamSource) throws DatasetBuilderException {
    ...
    private static <T> Dataset<T> getBatchDataset(BatchSource batchSource) throws DatasetBuilderException {
        return (Dataset<T>) SparkSourceFactory.<T>builder()
    ...
    public interface DatasetFactory<T> {
        Dataset<T> get() throws DatasetBuilderException;
    }
