...
public class ScalaScriptPartitionTransformation<S, D> implements
    ...
    public Dataset<D> apply(Dataset<S> dataset) {
        var map = new ScalaScriptMapPartitionFunction<S, D>(scalaScriptParameters.getCode(), transformationContextBroadcast);
        return dataset.mapPartitions(map, Objects.requireNonNull(encoder, "encoder is null"));
    }
