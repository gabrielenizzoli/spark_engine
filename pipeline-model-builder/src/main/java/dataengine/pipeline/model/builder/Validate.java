...
public class Validate {
    ...
    public static Consumer<List> listSize(String msg, Integer min, Integer max) {
        return (in) -> {
            if (in == null || in.size() == 0)
                throw new DataFactoryException(msg + " list is null or empty");
            if (min != null && in.size() < min)
                throw new DataFactoryException(msg + " list size is less than " + min);
            if (max != null && in.size() > max)
                throw new DataFactoryException(msg + " list size is more than " + max);
        };
    }
    ...
    public static <T> Consumer<T> notNull(String msg) {
        return (in) -> {
            if (in == null)
                throw new DataFactoryException(msg + " is null");
        };
    }
    ...
    public static Consumer<String> notBlank(String msg) {
        return (in) -> {
            if (StringUtils.isBlank(in))
                throw new DataFactoryException(msg + " is blank");
        };
    }
