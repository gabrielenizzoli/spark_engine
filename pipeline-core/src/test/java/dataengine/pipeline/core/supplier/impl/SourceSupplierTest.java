...
class SourceSupplierTest extends SparkSessionBase {

    @Test
    void getTestDatasetAsText() {
        DatasetSupplier<String> ds = SourceSupplier.builder().text("src/test/resources/datasets/testDataset.json").batch().build();
        Assertions.assertEquals(6, ds.get().count());
    }

    @Test
    void getTestDatasetAsJson() {
        DatasetSupplier<Row> ds = SourceSupplier.builder().json("src/test/resources/datasets/testDataset.json").batch().build();
